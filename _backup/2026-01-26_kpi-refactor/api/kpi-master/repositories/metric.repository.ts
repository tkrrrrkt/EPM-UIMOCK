...
export class MetricRepository {
  ...
  /**
   * Find all metrics with kpi_managed=true
   *
   * @param tenantId - Tenant ID (required)
   * @returns Array of metrics with kpi_managed=true
   */
  async findManyByKpiManaged(
    tenantId: string,
  ): Promise<Array<{ id: string; metricCode: string; metricName: string; kpiManaged: boolean }>> {
    // Set tenant context for RLS
    await this.prisma.setTenantContext(tenantId);
