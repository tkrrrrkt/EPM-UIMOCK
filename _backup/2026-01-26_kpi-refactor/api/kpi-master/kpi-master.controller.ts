...
export class KpiMasterController {
  ...
  async getEvents(
    ...
    const parsedQuery: GetKpiMasterEventsApiQueryDto = {
    ...
    return this.eventService.findAll(tenantId, parsedQuery);
  }
  ...
  async getItems(
    ...
    // Convert string query params to correct types
    const parsedQuery: GetKpiMasterItemsApiQueryDto = {
      offset: query.offset ? Number(query.offset) : 0,
      limit: query.limit ? Number(query.limit) : 50,
      eventId: query.eventId,
      parentKpiItemId: query.parentKpiItemId,
      kpiType: query.kpiType,
      hierarchyLevel: query.hierarchyLevel ? (Number(query.hierarchyLevel) as 1 | 2) : undefined,
      keyword: query.keyword,
      sortBy: query.sortBy,
      sortOrder: query.sortOrder,
    };
    return this.itemService.findAll(tenantId, parsedQuery);
  }
  ...
  /**
   * GET /kpi-master/selectable-subjects
  ...
  async getSelectableSubjects(
    ...
    const subjects = await this.selectableOptionsService.findSelectableSubjects(
      tenantId,
      companyId,
    );
    return { subjects };
  ...
  /**
   * GET /kpi-master/selectable-metrics
  ...
  async getSelectableMetrics(
    ...
    const metrics = await this.selectableOptionsService.findSelectableMetrics(
      tenantId,
      companyId,
    );
    return { metrics };
