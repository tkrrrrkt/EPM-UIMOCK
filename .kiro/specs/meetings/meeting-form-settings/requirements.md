...
本フォーム設定は**議事録タブのフォーム**としても使用される。

**戦略**：議事録用セクションを報告フォーム設定内に含める
- 議事録用セクションのコード: `sectionCode='MEETING_MINUTES'` で固定
- 議事録タブでは、このセクションに属するフィールドのみを表示
- 部門報告の入力スコープ（DEPARTMENT/BU/COMPANY）は使用しない（全社共通フォーム）
- 議事録は会議開催**後**に管理者が登録する（部門報告は開催前に各部が登録）

**標準的な構成**:
| セクション | 用途 | input_scope |
|-----------|------|------------|
| SUMMARY | 部門報告サマリー | COMPANY/BU/DEPARTMENT |
| KPI_FORECAST | KPI見通し | COMPANY/BU/DEPARTMENT |
| MEETING_MINUTES | 議事録（固定） | COMPANY |
