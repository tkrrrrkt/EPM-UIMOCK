...
import { CreateForecastEventDialog } from "../dialogs/create-event-dialog"
import { DuplicateForecastEventDialog } from "../dialogs/duplicate-event-dialog"
...
export function ForecastEventListPage() {
  const [events, setEvents] = useState<BffForecastEventSummary[]>([])
  const [loading, setLoading] = useState(true)
  const [fiscalYearFilter, setFiscalYearFilter] = useState<string>("ALL")
  const [createDialogOpen, setCreateDialogOpen] = useState(false)
  const [duplicateDialogOpen, setDuplicateDialogOpen] = useState(false)
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false)
  const [selectedEvent, setSelectedEvent] = useState<BffForecastEventSummary | null>(null)
  ...
  function handleDuplicate(event: BffForecastEventSummary) {
    setSelectedEvent(event)
    setDuplicateDialogOpen(true)
  }

  function handleDelete(event: BffForecastEventSummary) {
    setSelectedEvent(event)
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error && error.message === "FORECAST_EVENT_HAS_FIXED_VERSION"
    ...
    <div className="min-h-screen bg-background p-6">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-balance text-3xl font-bold tracking-tight text-foreground">見込入力</h1>
            <p className="text-pretty mt-2 text-muted-foreground">
              月次見込イベントを管理し、科目×月の見込データを入力
            </p>
          </div>
                  ...
                  {loading ? (
                    ...
                    events.map((event) => (
                      <TableRow
                        ...
                        onClick={() => (window.location.href = `/transactions/forecast-entry/${event.id}`)}
                      >
      ...
      {/* Dialogs */}
      <CreateForecastEventDialog open={createDialogOpen} onOpenChange={setCreateDialogOpen} onSuccess={loadEvents} />

      {selectedEvent && (
        <>
          <DuplicateForecastEventDialog
