...
interface VersionCompareGridProps {
  data: BffBudgetCompareResponse | null
  loading: boolean
}

export function VersionCompareGrid({ data, loading }: VersionCompareGridProps) {
  ...
  if (loading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">バージョン比較</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  // 月グループ化レイアウト: 4月ベース,4月現在,4月差異,5月ベース...
  ...
  const renderTableRow = (row: BffBudgetCompareRow, periods: BffPeriodColumn[]) => {
    ...
    return (
      ...
      >
        ...
        {/* 期間ごとに ベース・現在・差異 を横並び */}
        {periods.map((period, idx) => {
          const baseCell = row.baseCells[idx]
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <p className="text-sm text-muted-foreground">
            {data.context.departmentName} / {data.context.fiscalYear}年度 / {data.context.planVersionName}
          </p>
        </div>
      </CardHeader>
