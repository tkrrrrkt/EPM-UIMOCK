...
interface TreeGridBudgetGridProps {
...
// Convert BffBudgetRow to TreeGrid format
...
export function TreeGridBudgetGrid({
  periods,
  rows,
  isEditable,
  onCellChange,
  onToggleExpand,
}: TreeGridBudgetGridProps) {
  ...
  const handleCellSave = React.useCallback(
    ...
    async (args: any) => {
      ...
      // Check editability
      if (!row.isEditable || !period.isEditable) {
        args.cancel = true
        return
      }
  ...
  const handleCellEdit = React.useCallback(
    ...
    (args: any) => {
      ...
      const period = periods.find((p) => p.periodId === columnName)
      if (!period || !period.isEditable || !period.isOpen) {
        args.cancel = true
        return
      }
    ...
    (args: any) => {
      ...
      if (period) {
        ...
        if (period.periodType === "QUARTER") {
          cell.style.backgroundColor = isAggregate ? "#e0f2fe" : "#f0f9ff"
        } else if (period.periodType === "HALF") {
          cell.style.backgroundColor = isAggregate ? "#bfdbfe" : "#dbeafe"
          cell.style.fontWeight = "600"
  ...
  return (
    <div className="relative">
        ...
        <ColumnsDirective>
          ...
          {periods.map((period) => (
            <ColumnDirective
              ...
              textAlign="Right"
              format="N0"
              allowEditing={period.isEditable && period.isOpen}
              clipMode="Clip"
            />
          ))}
        </ColumnsDirective>
      ...
      {isEditable && (
        <div className="flex items-center justify-center gap-6 px-3 py-2 bg-muted/20 border-t text-[11px] text-muted-foreground">
          ...
          <span><kbd className="px-1 py-0.5 bg-muted rounded text-[10px]">Ctrl+C</kbd> コピー</span>
          <span><kbd className="px-1 py-0.5 bg-muted rounded text-[10px]">Ctrl+V</kbd> ペースト</span>
        </div>
      )}
    </div>
  )
