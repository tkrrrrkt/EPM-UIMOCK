...
interface AllocationExecuteDialogProps {
  open: boolean
  ...
  onLoadEvents: () => Promise<BffAllocationEvent[]>
  onExecute: (eventIds: string[]) => Promise<BffExecuteAllocationResponse>
  onSuccess: () => void
  loading?: boolean
}

export function AllocationExecuteDialog({
  open,
  onOpenChange,
  periodLabel,
  companyId,
  ...
  onSuccess,
  loading,
}: AllocationExecuteDialogProps) {
  const [events, setEvents] = useState<BffAllocationEvent[]>([])
  const [selectedEventIds, setSelectedEventIds] = useState<string[]>([])
  const [isLoadingEvents, setIsLoadingEvents] = useState(false)
  const [isExecuting, setIsExecuting] = useState(false)
  const [executeResult, setExecuteResult] = useState<BffExecuteAllocationResponse | null>(null)
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      if (result.success) {
        // Wait a moment to show success, then close and navigate
        setTimeout(() => {
          onOpenChange(false)
          onSuccess()
        }, 1500)
                    ...
                    >
                      <TableCell>
                        <Checkbox
                          ...
                          disabled={!event.isActive || loading || isExecuting}
                        />
                      </TableCell>
          ...
          {!executeResult && (
            <>
              ...
              <Button
                ...
                disabled={loading || isExecuting || selectedEventIds.length === 0}
              >
