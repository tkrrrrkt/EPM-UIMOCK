...
import { TrendingUp, TrendingDown, Minus, BarChart3, LineChart as LineChartIcon, Building2, Globe } from "lucide-react"
import {
  BarChart,
  Bar,
  LineChart,
...
import type { BffMtpEventDetailResponse, BffMtpTrendResponse } from "@epm/contracts/bff/mtp"

const bffClient = new MockBffClient()

type ChartType = "bar" | "line" | "composed"

interface MtpTrendTabProps {
  eventId: string
  event: BffMtpEventDetailResponse
}

export function MtpTrendTab({ eventId, event }: MtpTrendTabProps) {
  const [trendData, setTrendData] = useState<BffMtpTrendResponse | null>(null)
  ...
  const chartData = useMemo(() => {
    ...
    const yearMap = new Map<number, { year: string; fiscalYear: number; actual?: number; plan?: number; growth?: number }>()
    ...
    trendData.dataPoints.forEach((point) => {
      ...
      const amount = point.amount / 100000000 // 億円換算
    ...
    for (let i = 1; i < data.length; i++) {
      const current = data[i].actual || data[i].plan || 0
      const prev = data[i - 1].actual || data[i - 1].plan || 0
      if (prev > 0) {
  ...
  const kpiMetrics = useMemo(() => {
    ...
    const latestPlan = chartData.filter(d => d.plan !== undefined).pop()
    const allValues = chartData.map(d => d.actual || d.plan || 0)
    const totalPlan = chartData.filter(d => d.plan !== undefined).reduce((sum, d) => sum + (d.plan || 0), 0)

    // CAGR計算 (Compound Annual Growth Rate)
    const firstValue = chartData[0]?.actual || chartData[0]?.plan || 0
    const lastValue = chartData[chartData.length - 1]?.actual || chartData[chartData.length - 1]?.plan || 0
    const years = chartData.length - 1
    const cagr = years > 0 && firstValue > 0 ? (Math.pow(lastValue / firstValue, 1 / years) - 1) * 100 : 0

    return {
      latestActualValue: latestActual?.actual || 0,
      latestActualYear: latestActual?.fiscalYear || 0,
      latestPlanValue: latestPlan?.plan || 0,
      latestPlanYear: latestPlan?.fiscalYear || 0,
      maxValue: Math.max(...allValues),
      totalPlan,
      cagr,
      planYearsCount: chartData.filter(d => d.plan !== undefined).length,
    }
  }, [chartData])
        ...
        <div className="flex flex-wrap items-end gap-4">
          <div className="space-y-1.5">
            ...
            <Select value={selectedSubjectId} onValueChange={setSelectedSubjectId}>
              ...
              <SelectContent>
                <SelectItem value="subj-001">売上高</SelectItem>
                <SelectItem value="subj-002">売上原価</SelectItem>
                <SelectItem value="subj-003">売上総利益</SelectItem>
                <SelectItem value="subj-004">販管費</SelectItem>
                <SelectItem value="subj-005">営業利益</SelectItem>
              </SelectContent>
            </Select>
          </div>
      ...
      {kpiMetrics && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card className="bg-gradient-to-br from-emerald-50 to-emerald-100/50 dark:from-emerald-950/20 dark:to-emerald-900/10 border-emerald-200/50">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-emerald-700 dark:text-emerald-400">最終年度計画 (FY{kpiMetrics.latestPlanYear})</p>
              </div>
              <p className="mt-2 text-3xl font-bold text-emerald-900 dark:text-emerald-100">
                {kpiMetrics.latestPlanValue.toFixed(1)}
          ...
          <Card className="bg-gradient-to-br from-purple-50 to-purple-100/50 dark:from-purple-950/20 dark:to-purple-900/10 border-purple-200/50">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-purple-700 dark:text-purple-400">計画期間合計 ({kpiMetrics.planYearsCount}年)</p>
              </div>
              <p className="mt-2 text-3xl font-bold text-purple-900 dark:text-purple-100">
                {kpiMetrics.totalPlan.toFixed(1)}
          ...
          <Card className="bg-gradient-to-br from-amber-50 to-amber-100/50 dark:from-amber-950/20 dark:to-amber-900/10 border-amber-200/50">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-amber-700 dark:text-amber-400">年平均成長率 (CAGR)</p>
                {getGrowthIcon(kpiMetrics.cagr)}
              </div>
              <p className={cn("mt-2 text-3xl font-bold", getGrowthColor(kpiMetrics.cagr))}>
                {kpiMetrics.cagr >= 0 ? "+" : ""}{kpiMetrics.cagr.toFixed(1)}
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-xl">
                {trendData.subject.subjectName}の推移
              </CardTitle>
              <CardDescription className="text-base mt-1">
                {trendData.dimensionValue ? trendData.dimensionValue.valueName : "全社"} | 実績 + 計画
              </CardDescription>
            </div>
          </div>
        </CardHeader>
              ...
              <ComposedChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                ...
                <Tooltip
                  ...
                  formatter={(value, name) => {
                    ...
                    if (name === "growth") return [`${value >= 0 ? "+" : ""}${value.toFixed(1)}%`, "成長率"]
                    return [`${value.toFixed(1)}億円`, name === "actual" ? "実績" : "計画"]
                  }}
                />
                <Legend wrapperStyle={{ paddingTop: "20px" }} />
                <Area
                  ...
                  name="実績"
                />
                <Bar dataKey="plan" fill="hsl(var(--chart-2))" name="計画" radius={[4, 4, 0, 0]} opacity={0.8} />
                <Line
                  type="monotone"
                  dataKey="growth"
            ...
            <table className="w-full text-base">
              <thead>
                <tr className="border-b bg-muted/40">
                  ...
                  <th className="p-4 text-center font-semibold min-w-[100px]">種別</th>
              ...
              <tbody>
                {chartData.map((row, idx) => {
                  const value = row.actual || row.plan || 0
